...
def scrape(url: str, item_list: list):
    ...
    for item in item_list:
        ...
        item_info = get_info_by_item_status(int(avail_qty), int(backorder_qty), int(open_order_qty))
        output.append((str(item), avail_qty, on_hand_qty, commit_qty, backorder_qty,
                       open_order_qty, item_status, item_info[0], item_info[1]))
    return output
...
def list_to_excel(item_list):
    ...
    dot_sheet = wb.active
    header = ["Item Number", "# Avail", "# Commit", "# OH", "# BO", "# PO", "Status", "Notes", "Contact"]
    dim_dict = {"A": 18, "B": 7, "C": 9, "D": 5, "E": 5, "F": 5, "G": 10, "H": 21, "I": 7}
